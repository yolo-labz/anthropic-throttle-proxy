...
| `THROTTLE_ACCOUNT_ROUTING` | `off` | `off` / `least_loaded`. With `least_loaded`, a local proxy reads `THROTTLE_ACCOUNT_CRED_PATHS`, chooses the least-loaded usable account for each `POST /v1/messages`, and rewrites only the upstream `Authorization` header. Raw tokens are never logged or exported. Keep off on Dokku/central tiers that do not have local credential files. |
