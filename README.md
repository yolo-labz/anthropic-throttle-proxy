...
| `THROTTLE_QUEUE_DRAIN_DEFAULT_S` | `10` | Service-time estimate used by queue-DEPTH admission before a lane has completed enough requests to measure its own. Bounding the wait does not bound the depth: a queue deeper than the lane can drain at its own service rate parks a request that provably cannot be served (measured 01/09/2026 on the `:8766` lane — `max_concurrent=2`, depth 3, 30 s budget, occupied slots completing after 113.7 s and 221.2 s). Such a request is now refused BEFORE it is parked, with the drain arithmetic in the log line and in `/__throttle/admission`. Enforcement requires evidence — three completions, or one slot held past this default — so a lane that has measured nothing never rejects on a guess. |
| `THROTTLE_QUEUE_RETRY_AFTER_MAX_S` | `300` | Ceiling on the `Retry-After` advertised with a queue-timeout `503`. The value is the lane's drain estimate, never the old constant `5`, and is never shorter than `THROTTLE_QUEUE_MAX_WAIT_S`-worth of patience the client just spent — a shorter hint sends a compliant client straight back into the same wall (measured as four retries and an abort). |
