...
| `THROTTLE_QUEUE_MAX_WAIT_S` | `30` | Longest a request may wait in the fair queue for a slot before failing fast with `503` + `Retry-After` + `x-anthropic-throttle-queue-timeout: 1`. Claude Code aborts a silent request at ~60 s; a response written after that hits a closing transport and surfaces as `InvalidHTTPResponse` (a phantom 401/login error). Answering inside the client's patience window makes the SDK retry transparently instead. A local tier relays the stamped 503 verbatim — no pushback retry, no AIMD shrink. `0` disables the bound. |
