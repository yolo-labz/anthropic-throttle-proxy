...
| `THROTTLE_QUEUE_MAX_WAIT_S` | `30` | Longest a request may wait in the fair queue for a slot before failing fast with `503` + `Retry-After` + `x-anthropic-throttle-queue-timeout: 1`. Claude Code aborts a silent request at ~60 s; a response written after that hits a closing transport and surfaces as `InvalidHTTPResponse` (a phantom 401/login error). Answering inside the client's patience window makes the SDK retry transparently instead. The bound is END-TO-END: each tier forwards the remaining budget via `x-anthropic-throttle-wait-budget-ms` and the next tier takes `min(own knob, inherited)`, so local + central waits cannot stack past the client's patience. A local tier relays the stamped 503 verbatim — no pushback retry, no AIMD shrink — and the stamp is only trusted from marker-bearing proxy responses (a raw upstream 503 carrying it is stripped). `0` disables the bound. |
| `THROTTLE_QUEUE_DRAIN_DEFAULT_S` | `10` | Service-time estimate used by queue-DEPTH admission before a lane has completed enough requests to measure its own. Bounding the wait does not bound the depth: a queue deeper than the lane can drain at its own service rate parks a request whose estimated wait is already past the budget (measured 01/09/2026 on the `:8766` lane — `max_concurrent=2`, depth 3, 30 s budget, occupied slots completing after 113.7 s and 221.2 s). Such a request is now refused BEFORE it is parked, with the drain arithmetic in the log line and in `/__throttle/admission`. The arrival's position is its round-robin rank, not raw queue depth, so a new client is not refused for a chatty client's backlog. Enforcement requires evidence — three completions, or one slot held past this default — so a lane that has measured nothing never rejects on a guess. |
| `THROTTLE_QUEUE_RETRY_AFTER_MAX_S` | `300` | Ceiling on the `Retry-After` advertised with a queue-timeout `503`. The value is the lane's drain estimate, never the old constant `5` (which stays as the floor). A request refused BEFORE it waited is additionally floored at the budget it was refused against — a shorter hint sends a compliant client straight back into the same wall (measured as four retries and an abort); one that already SPENT that budget waiting is not, because that time is behind it and a slot may free the moment after the timeout. This ceiling is a hard cap in both cases. |
...
| `THROTTLE_CHAT_MAX_BODY_BYTES` | `1800000` | Z.AI coding chat-completions only. A body over this many bytes has its oldest turns dropped until it fits, because a 707k-token seat (~3 MB) was refused with `413 Failed to buffer the request body: length limit exceeded` and that ceiling is not published: `/models` reports only the id and `owned_by`, the model's documentation advertises 1M tokens, a filed issue has the official API reporting 200K, and our own request died at 707k. The default sits deliberately **below** the plausible gate band — that 413 text is Bun's own body-buffering error (reported elsewhere at ~2.1–2.4 MB bodies) and axum's `DefaultBodyLimit` defaults to 2 MB — because a body fitted to just under a *wrong* ceiling still dies while one fitted a little too far succeeds. The system prompt is an anchor and is never dropped, the last `THROTTLE_CHAT_KEEP_TAIL` turns stay intact, and the cut never lands between an assistant tool call and the answer to it (an orphaned `role:"tool"` turn is an invalid transcript, so the proxy would be causing the refusal it prevents). Sizing costs one small serialization per turn, not a whole-body dump per probe — a 3 MB body would otherwise burn ~10 full dumps on the event loop. If even that cannot fit, the ORIGINAL body is forwarded unchanged: a lane that refuses a big body should refuse it, not receive something mangled. Both outcomes are counted (`anthropic_chat_body_fitted_total`, `anthropic_chat_body_unfittable_total`) and logged, so a fitted counter reading green can never hide a 413 still happening. Note the trade: a session past this budget is re-trimmed every turn with a shifting prefix, so its prompt cache stops being reusable — working-but-clipped beats broken, and the log's `normalized=`/`final=`/`turns_dropped=` fields are what will let the real ceiling be measured out of fleet data rather than guessed. `0` disables the trim. |
| `THROTTLE_CHAT_KEEP_TAIL` | `6` | Trailing turns the chat budget trim never drops — the model must still see the live turn and the exchange before it, or it resumes with no idea what it was doing. Minimum 2. |
...
| `THROTTLE_PRIORITY_RESERVE_SLOTS` | `0` | Dedicated dispatch pool for short/latency-sensitive calls (Stop-hook evaluators: small `max_tokens`, no tools, small body), independent of the main AIMD pool so they never queue-starve behind long generations. Total upstream concurrency ≤ live cap + this. Default `0` keeps storm traffic inside the live account cap; raise only with fresh evidence that evaluator latency is the binding problem. |
...
| `THROTTLE_ACCOUNT_CRED_PATHS` | *(unset)* | Local-only account map, `LABEL:/path/to/.credentials.json,...`, used by `/ui` to label bearer hashes and by account routing when enabled. |
| `THROTTLE_ACCOUNT_ROUTING` | `off` | `off` / `least_loaded` / `budget_paced`. With `least_loaded`, a local proxy reads `THROTTLE_ACCOUNT_CRED_PATHS`, chooses the least-loaded usable account for each `POST /v1/messages`, and rewrites only the upstream `Authorization` header. `budget_paced` keeps the same safety gates, but ranks usable accounts by reset-aware 5h/7d/scoped-window budget pressure so near-reset accounts with slack can be spent while early-cycle hot windows are protected. Raw tokens are never logged or exported. Keep off on Dokku/central tiers that do not have local credential files. |
| `THROTTLE_API_KEY_FILE` | *(unset)* | Runtime file containing one Anthropic API key for metered overflow. The key is read from disk, never from Nix/store or logs; selected requests are sent upstream with `x-api-key` and no OAuth `Authorization`. |
