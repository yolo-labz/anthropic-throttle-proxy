...
- **Queue-depth admission** (01/09/2026) — the wait bound above answers *how long* a request may wait, never *whether the wait is possible*. Before parking a request, `_FairSlotContext.__aenter__` asks `FairBearerLimiter.drain_estimate(max_wait)` whether the lane can drain one more arrival: with `free` idle slots and `slots` servers, the arrival's position is its **round-robin** rank (`_ahead_of`: own parked depth + `min(depth_j, own+1)` per sibling client — NOT raw queue depth, since a new client overtakes a chatty client's backlog by design) and it waits `ceil((position - free) / slots)` service rounds. The per-round service time is `max(p90 of the last 64 completions, elapsed hold of the oldest currently-held slot)`, falling back to `THROTTLE_QUEUE_DRAIN_DEFAULT_S` (10 s) before three completions exist; the FIRST round is priced at `residual_s` (what the current wave has left) so an arrival behind nearly-finished slots is not charged a whole round. Held slots are tracked by per-slot LEASE (`_holds`), not by arrival order — FIFO pairing let a stream of short completions pop the long holder's stamp and erase the one signal the incident turns on (Codex round-1 BLOCKER). Over the bound → `QueueWaitTimeout(pre_queue=True)` before enqueue, so the existing handler path answers the same stamped 503 (marker, anti-spoof, pushback-retry and AIMD exemptions all inherited unchanged) while the transport is wide open. **Enforcement requires evidence** (`DrainEstimate.rejects` = `enforced and evidenced and not admits`): a lane that has completed nothing and holds no slot past the cold default has GUESSED, and a guess must never turn traffic away — cold-start bursts behave exactly as before. The queue-timeout `Retry-After` is now that drain estimate (integer, floored at `QUEUE_TIMEOUT_RETRY_AFTER_S`, HARD-capped by `THROTTLE_QUEUE_RETRY_AFTER_MAX_S`) instead of the constant `5`; a request refused BEFORE it waited is additionally floored at the budget it was refused against (`budget_floored_retry_after`), while one that already spent that budget waiting is not — that time is behind it. Reason: 01/09/2026 14:20-14:24 BRT the `:8766` Z.AI lane had `max_concurrent=2`, depth 3, an inherited 30 s budget, and slots completing after 113.7 s / 221.2 s — the arrival needed two service rounds (~227 s) and could not be served by construction, yet was parked, timed out silently, and told to retry in 5 s; Pi retried the same wall four times and aborted. `/__throttle/admission` publishes `saturation.queue_admit_max_depth` (sum over measured bearers, config-derived when none are measured) plus `saturation.queue_admit` inputs, so consumers read saturation instead of re-deriving it. Service-time stamps are paired FIFO inside the limiter (`_note_dispatch`/`_note_completion`, both deques `maxlen`-bounded); a slot cancelled in the dispatch race is unstamped WITHOUT a sample, since it never reached upstream.
