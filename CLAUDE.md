...
- **OAuth entitlement 429 (17/08/2026)** — Anthropic refuses a *subscription* (OAuth) bearer whose `POST /v1/messages` does not carry the Claude Code identity as its **first `system` block**, and masks the refusal as `429 {"type":"error","error":{"type":"rate_limit_error","message":"Error"}}` with **no** `anthropic-ratelimit-unified-*` headers and **no** `Retry-After`. Haiku is exempt, which is why the failure reads as "Anthropic is shedding premium models" — measured 8/8 alternating trials across accounts A/B/C, seconds apart: same token, same model, CC block present → `200` + unified headers; absent or not first → `429`. `_is_oauth_entitlement_refusal` (all four signals must agree) makes the proxy relay it **immediately**, stamped `x-anthropic-throttle-oauth-entitlement: 1`, counted by `anthropic_oauth_entitlement_refusals_total`, and exempt from pushback-retry (`_should_retry_pushback`), keepalive-hold (`_is_transient_throttle`), and AIMD shrink/pause (`_aimd_feedback`). Waiting cannot change a request's shape, so the previous "headerless 429 → assume budget" fallback turned an instant client error into a 61 s stall **and** collapsed a healthy bearer: three `max_tokens=1` diagnostic probes drove `max_concurrent` 5→1 on two `allowed_warning` bearers that were still serving 200s, throttling the whole fleet. Genuine budget/concurrency 429s are untouched — they always carry unified headers, a `Retry-After`, or a descriptive message.
